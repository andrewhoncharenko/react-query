...
export default function EditEvent() {
  ...
  const { mutate } = useMutation({
    ...
    onError: (error, data, context) => {
      queryClient.setQueryData(["event", params.id], content.previousEvent);
    },
    onSettled: () => {
  ...
  if(isError) {
    content = <>
      <ErrorBlock title="Failed to load event." message={error.info?.message || "Failed to load event. Please check your inputs and try again."}/>
